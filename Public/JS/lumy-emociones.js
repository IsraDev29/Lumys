 ...
 * ---------------------------------------------------------------------------
 * Fuente de verdad única. De este archivo salen tres cosas:
 *
 *   1. El rig que corre en la PWA          (Public/JS/lumy.js)
 *   2. Los SVG por emoción para importar   (tools/lumy-exportar-svg.js)
 *   3. La especificación del rig de Rive   (docs/lumy-rive-spec.md)
 *
 * Si una emoción se retoca acá, las tres salidas cambian juntas. No hay
 ...
 *
 * `lumy.js` hace cumplir esto en código: `setEmocion()` rechaza una emoción de
 * banda 'espejo' salvo que se pase `{ espejo: true }` explícito.
 * =========================================================================== */

(function (raiz, fabrica) {
  const api = fabrica();
  if (typeof module === 'object' && module.exports) module.exports = api;
  else raiz.LumyEmociones = api;
  ...
  const GEO = {
    ...
    centroCuerpo: [125, 115],   // centro del bounding box del blob
    baseCuerpo:   [125, 186],   // borde inferior: pivote para squash con "pie" fijo
    ...
    rubor: [ { cx: 76, cy: 134, rx: 13, ry: 7.5 },
             { cx: 164, cy: 134, rx: 13, ry: 7.5 } ],

    ojos: { izq: [96, 113], der: [144, 113], rx: 12, ry: 14 },
    ...
    chispas: [ { x: 206, y: 64,  s: 10.5, color: '#C2410C', op: 1   },
               { x: 32,  y: 82,  s: 6,    color: '#6FCBBB', op: 1   },
               { x: 196, y: 172, s: 5,    color: '#C2410C', op: 0.5 } ],
  };
  ...
  const PALETAS = {
    ...
    mentaAlta:{ cuerpo: ['#FBFFFD', '#E6FBF4', '#C6F2E2', '#7FDCC2', '#31B392'], tinta: '#065F46', halo: '#A6E8D5', rim: '#12857A' },
    ...
    musgo:    { cuerpo: ['#F9FBF0', '#EAF2D8', '#D0DEAE', '#A3B878', '#6F8545'], tinta: '#3C4A22', halo: '#C3D49E', rim: '#566A32' },
    ...
    amanecer: { cuerpo: ['#FFFCF3', '#EFF9E8', '#C9EDD3', '#87CFB0', '#3FA383'], tinta: '#0A5A46', halo: '#A8DEC4', rim: '#147B5F' },
  };
  ...
  const BASE = {
    // --- Cuerpo -------------------------------------------------------
    cuerpoX: 0, cuerpoY: 0,        // traslación, unidades de usuario
    cuerpoSX: 1, cuerpoSY: 1,      // squash / stretch
    cuerpoRot: 0,                  // grados
    cuerpoPivote: 0,               // 0 = centro del blob, 1 = base del blob

    // --- Cara (grupo entero: rubor + ojos + boca) ----------------------
    caraX: 0, caraY: 0, caraRot: 0, caraS: 1,

    // --- Ojos ---------------------------------------------------------
    ojoSX: 1, ojoSY: 1,            // tamaño del ojo
    ojoAbre: 1,                    // párpado superior. 1 abierto, 0 cerrado
    ojoAbajo: 0,                   // párpado inferior (entrecerrar)
    ojoAng: 0,                     // inclinación del párpado. + enojo, − preocupación
    ojoX: 0, ojoY: 0,              // dirección de la mirada
    ojoSep: 0,                     // separación extra entre ojos
    ojoArco: 0,                    // 0..1 fundido al ojo cerrado en arco
    ojoArcoC: 1,                   // curvatura del arco. +1 feliz (∩), −1 triste (∪)
    ojoBrillo: 1,                  // opacidad de los reflejos

    // --- Boca ---------------------------------------------------------
    bocaCurva: 1,                  // 1 = la sonrisa original. 0 recta, − hacia abajo
    bocaAncho: 1,
    bocaAbre: 0,                   // 0..1 apertura (boca rellena)
    bocaOnda: 0,                   // 0..1 temblor / zigzag
    bocaX: 0, bocaY: 0,
    bocaGrosor: 1,
    ...
    auraOp: 1, auraS: 1,
    brilloOp: 1,                   // brillos especulares del cuerpo

    // --- Ritmo (bucle de reposo) --------------------------------------
    flota: 4.5,                    // amplitud vertical del flotado, en unidades
    flotaVel: 5.5,                 // segundos por ciclo
    tiembla: 0,                    // amplitud del temblor
    respira: 0.012,                // amplitud del "respirado" (escala)
    respiraVel: 4,                 // segundos por ciclo
    parpadeoVel: 5.2,              // segundos promedio entre parpadeos

    // --- Capas de efecto (todas 0 en reposo) ---------------------------
    fxLagrima: 0,                  // lágrimas
    fxSudor: 0,                    // gota de nervios
    fxEspiral: 0,                  // espirales de confusión
    fxConfeti: 0,                  // celebración
    fxCorazon: 0,                  // empatía / cariño
    fxVapor: 0,                    // vapor de enojo / agobio
    fxLineas: 0,                   // líneas de impacto (sorpresa)
    fxNube: 0,                     // nubecita de lluvia
  ...
  const EMOCIONES = {
    ...
    alegria: {
      ...
      pose: {
        paleta: 'mentaAlta', ojoAbajo: 0.32, ojoSY: 0.92, bocaCurva: 1.45,
    ...
    aversion: {
      ...
      pose: {
        paleta: 'musgo', ojoAbre: 0.52, ojoAng: 1.8, ojoSY: 0.85, ojoX: -2.5,
    ...
    repulsion: {
      ...
      pose: {
        paleta: 'musgo', ojoAbre: 0.4, ojoAng: 3, ojoSY: 0.72, ojoX: -3.5,
    ...
    anticipacion: {
      ...
      pose: {
        paleta: 'mentaAlta', ojoSX: 1.14, ojoSY: 1.16, ojoY: -1.5, ojoBrillo: 1.2,
   ...
   * ------------------------------------------------------------------- */
  const CLAVES = Object.keys(EMOCIONES);
  ...
  const APOYO = CLAVES.filter((c) => EMOCIONES[c].banda === 'apoyo');
  ...
  function pose(clave) {
    ...
    return Object.assign({}, BASE, e.pose);
  }
  ...
  return {
    GEO, PALETAS, BASE, EMOCIONES, FAMILIAS, CLAVES, APOYO,
    POR_CLIMA, porIcve, pose,
    VERSION: '1.0.0',
  };
});
