/* ==========================================================================
   ...
   El módulo no cambió al pasar a React porque no tenía nada de interfaz: son
   Web Audio y aritmética. Lo que sí se agregó es `useNotaDeVoz`, el hook que lo
   envuelve para la vista del check-in.
   ========================================================================== */
...
export type Prosodia = {
  ...
  proporcionPausas: number;
  variacionTono: number;
  palabrasPorMinuto: number;
  energiaMedia: number;
  tonoMedio: number;
};

type Muestra = { tono: number | null; rms: number; hablando: boolean };
 ...
 */
function estimarTono(buffer: Float32Array, frecuenciaMuestreo: number): { tono: number | null; rms: number } {
  const n = buffer.length;

  let energia = 0;
  for (let i = 0; i < n; i += 1) energia += (buffer[i] ?? 0) ** 2;
  ...
  for (let desfase = desfaseMin; desfase <= desfaseMax; desfase += 1) {
    ...
    for (let i = 0; i < n - desfase; i += 1) {
      correlacion += (buffer[i] ?? 0) * (buffer[i + desfase] ?? 0);
    }
...
function resumir(): Prosodia | null {
  const total = muestras.length;
  if (!total) return null;

  const conHabla = muestras.filter((m) => m.hablando);
  const tonos = muestras.map((m) => m.tono).filter((t): t is number => t !== null);
...
/** Se comprueba con `typeof` y no con una verdad simple: los tipos del DOM
 *  declaran `getUserMedia` como siempre presente, así que `&&` a secas era una
 *  condición constante para el compilador. En ejecución no lo es — falta en
 *  navegadores viejos y en contextos sin HTTPS. */
 ...
 */
export async function grabar(): Promise<void> {
 ...
 */
export function detener(): Prosodia | null {
  soltarMicrofono();
...
export const grabando = (): boolean => temporizador !== null;
